...
up_bytes = data.loc[(data['dst_ip'] == '200.0.0.11') | (data['dst_ip'] == "200.0.0.12")].groupby(['src_ip','dst_ip','port'])['up_bytes'].sum().sort_values(ascending=False)
down_bytes = data.loc[(data['dst_ip'] == '200.0.0.11') | (data['dst_ip'] == "200.0.0.12")].groupby(['src_ip','dst_ip','port'])['down_bytes'].sum().sort_values(ascending=False)
data['diff_timestamp'] = data.groupby(['src_ip','dst_ip','port'])['timestamp'].diff().fillna(0)
timestamp = data.groupby(['src_ip','dst_ip','port'])['diff_timestamp'].mean().sort_values(ascending=False)
